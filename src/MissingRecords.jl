...
export MissingBar, DataFrame, plot
...
function MissingBar(df::DataFrame)
    colnames = names(df)
    numrows = nrow(df)
    missing_num = [sum(ismissing.(df[!,i])) for i in colnames]
    missing_pct = round.( (missing_num ./ numrows) .* 100, digits=2)
    MissingBar(colnames, missing_num, missing_pct, numrows)
end


function DataFrame(mb::MissingBar)
...
Base.convert(::Type{DataFrame}, mb::MissingBar) = DataFrame(mb)
Base.show(io::IO, mime::MIME"text/plain", mb::MissingBar) = display(sort!(DataFrame(mb), :missing_num, rev=true))
